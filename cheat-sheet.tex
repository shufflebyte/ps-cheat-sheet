...
\ohead{LF10b - Lehrkraft: Soulier - Version 1.1.0-beta Schuljahr 2025/2026}%\includegraphics[height=1.7cm]{drslogo.png}}
...
\newif \ifregularexpressions \regularexpressionstrue
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Einstellungen %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Erste Spalte %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    \begin{tabular}{p{0.44\linewidth} p{0.56\linewidth}}
	    \rowcolor{lightblue}\texttt{Update-Help -UICulture en-US} & Als Admin: aktualisiert Hilfedateien \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Zweite Spalte %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{tabular}{p{0.4\linewidth} p{0.6\linewidth}}
	    \rowcolor{lightpink} \texttt{Set-Location} & In Pfad wechseln \\
	    \texttt{Get-Location} & Aktuellen Pfad holen \\
	    ...
	    \texttt{Set-Item} & Setzt Meta-Informationen\\
	    \rowcolor{lightpink} \texttt{Get-Content} & Liest Inhalt einer Datei ein\\
	    \texttt{Set-Content} & Ersetzt den Inhalt einer Datei \\
	    \rowcolor{lightpink} \texttt{Add-Content} & Fügt Inhalt in einer Datei an\\
	    \texttt{Get-ChildItem} & Holt ein Item und seine Kinder-Items (Unterordner)\\
	    \rowcolor{lightpink} \texttt{Tree} & Zeigt Ordner rekursiv in Baumstruktur (nicht PS)\\
	\end{tabular}
	\fi
	...
	\begin{tabular}{p{0.4\linewidth} p{0.6\linewidth}}
	    \texttt{Write-Output} & Erzeugt eine Ausgabe auf \texttt{stdout} \\
	...
	\begin{tabular}{p{0.4\linewidth} p{0.6\linewidth}}
		...
		\rowcolor{lightpink} \texttt{\$a -like \dq W*\dq} & Vergleich auf Wildcard-Pattern eines Strings \\
		 \texttt{\$a -in \$x} & Vergleich, ob in Liste enthalten \\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Zweite Spalte %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{tabular}{p{0.54\linewidth} p{0.46\linewidth}}
	    ...
	    \rowcolor{lightgreen}\texttt{\$x.replace(\dq ü\dq, \dq ue\dq)} & Case-Sensitives Ersetzen von Teilstrings \\
	    \texttt{\$x -replace \dq \textbackslash d \dq, \dq X\dq} & Ersetzten von Teilstrings m.H. von regulären Ausdrücken \\
	...
	\begin{tabular}{p{0.3\linewidth} p{0.7\linewidth}}
	    ...
	    \rowcolor{lightyellow}\texttt{\$\{arr\}[100]} & Falls \texttt{arr[100]} nicht existiert, gib \$null zurück \\
	\end{tabular}
	\fi
	\newpage
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Dritte Spalte %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\begin{tabular}{p{0.4\linewidth} p{0.6\linewidth}}
	    \rowcolor{lightyellow} \texttt{Write-Output} & Erzeugt Ausgabe auf \texttt{stdout} \\
	...
	\begin{tabular}{p{0.4\linewidth} p{0.6\linewidth}}
	    \rowcolor{lightpeach} \texttt{\$x = \dq a\dq,\dq b\dq,\dq c\dq} & Array definieren \\
	     \texttt{\$x = @(1,2,3)} & Array definieren \\
	    \rowcolor{lightpeach} \texttt{\$x = 1..10} & Zahlen 1 bis 10 in x schreiben \\
	    \texttt{\$x.Length} & Anzahl der Elemente holen \\
	    \rowcolor{lightpeach} \texttt{\$z = \$x + \$y} & Zwei Arrays verbinden \\ \hline
	    \texttt{\$x[-3..-1]} & Letzte 3 Elemente holen \\	    	    
	    \rowcolor{lightpeach} \texttt{\$x[1,4+6..9]} & Elemente 1,4,6 bis 9 holen \\ \hline
		
		\texttt{\$x = (\dq a\dq, \dq b\dq), (\dq c\dq, \dq d\dq)} & Zwei-dimensionales Array erzeugen \\
		\rowcolor{lightpeach} \texttt{\$x[0][1]} & Element \dq b\dq an $(0,1)$ holen  \\ \hline
		\texttt{\$x = @\{a = \dq w1\dq; b = \dq w2\dq\}} & Assoziatives Array erzeugen \\ 
		\rowcolor{lightpeach} \texttt{\$x[\dq a\dq]} bzw. \texttt{\$x.a} & Wert von Index \dq a\dq auslesen\\
		\texttt{\$Get-Help about\_Arrays} & Hilfeartikel zu Arrays aufrufen \\ 
	\end{tabular}
	\fi
	...
	\begin{tabular}{p{0.6\linewidth} p{0.4\linewidth}}
		...
		\rowcolor{lightyellow} \texttt{switch(\$x)~\{ 1~\{~\$y= \dq A\dq\}~ \linebreak 2~\{~\$y= \dq B\dq\} \linebreak \} } & Switch-Case Abfrage: x=1 ? dann y = \dq A\dq usw. \\
	...
	Alles was nicht \texttt{Write-*} ausgegeben wird, wird zum \textbf{Rückgabewert} der Funktion \\
	\begin{tabular}{p{0.6\linewidth} p{0.4\linewidth}}
	\rowcolor{lightgreen} \texttt{function Get-Alter()} & \textbf{positionale} Parameter \\
	\texttt{function Get-Alter(\$age)} & \textbf{benannte} Parameter \\
	\rowcolor{lightgreen}\begin{footnotesize} \texttt{function Get-Age([int]\$age)} \end{footnotesize}& \textbf{benannte} und typisierte Parameter \\
	\texttt{function Get-Alter() \{ param([int] \$age) \}} & Variante 2 \\ \hline
	\rowcolor{lightgreen} \texttt{Get-Alter 20} & Aufruf mit \textbf{positionalem} Parameter \\
	\texttt{Get-Alter -age 30} & Aufruf mit \textbf{benanntem} Parameter \\ \hline
	\rowcolor{lightgreen} \texttt{return} & Funktion (vorzeitig) verlassen. \\
	\end{tabular}
	\fi
	...
	\rowcolor{lightblue} \texttt{\$?} & \texttt{\$True} für Erfolg, \texttt{\$False} für Fehlschlag \\
	\texttt{<a> \&\& <b>} & Führt $<$b$>$ aus, falls $<$a$>$ erfolgreich \\
	\rowcolor{lightblue} \texttt{<a> || <b>} & Führt $<$b$>$ aus, falls $<$a$>$ \textbf{nicht} erfolgreich \\
	\texttt{try\{~\#unsafe \} catch\{~\#catchIt \} finally\{~\#after \}} & Try unsafe code, Catch error and finally do something \\
	\rowcolor{lightblue} \texttt{\$\_} & Zugriff auf Error-Objekt innerhalb des catch-Blocks \\
	\end{tabular}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Zweite Spalte %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\switchcolumn
	\begin{tabular}{p{0.43\linewidth} p{0.57\linewidth}}
		\rowcolor{lightblue} \texttt{Write-Error \dq Fehler bei ... \$\_\dq} & Fehlerausgabe im Catch-Block mit Zugriff auf Error-Objekt \\
		\texttt{throw \dq Fehler: Skript gestoppt} & Einen eigenen Fehler erzeugen \\
	\end{tabular}
	
	\textbf{Error-Action} und \textbf{Warning-Action}: \\\texttt{Stop}, \texttt{Continue}, \texttt{SilentlyContinue}, \texttt{Inquire} \\
	\begin{tabular}{p{1.05\linewidth }}
	    \rowcolor{lightblue} \texttt{Get-Help about\_CommonParameters} \\ 
	\end{tabular} 
	\ifregularexpressions 
	\subsubsection*{Reguläre Ausdrücke}
	\begin{tabular}{p{0.5\linewidth} p{0.5\linewidth}}	
	\rowcolor{lightpink} \texttt{\$z -match \dq Peter\dq} & Liefert \$true bzw. \$false zurück. Falls \$true steht Ergebnis in \$matches[0]\\ \hline
	\texttt{\$z -match \dq \^{}Peter\dq} & Anker Textbeginn \\
	\rowcolor{lightpink} \texttt{\$z -match \dq Peter\$\dq} & Anker Textende\\ \hline
	\texttt{\$z -match \dq \textbackslash w\dq} & Prüfe auf word: alphanumerische Zeichen \\
	\rowcolor{lightpink} \texttt{\$z -match \dq \textbackslash d\dq} & Digit\\
	\texttt{\$z -match \dq \textbackslash s\dq} & Whitespace \\
	\rowcolor{lightpink} \texttt{\$z -match \dq \textbackslash D \dq} & Not Digit (geht bei allen Character Classes)\\
	\texttt{\$z -match \dq [äöüß]\dq} & Character Set: z. B. nur dt. Umlaute finden \\ \hline
	\rowcolor{lightpink} \texttt{\$z -match \dq \textbackslash d\{4,\} \dq} & Quantifiers: mindestens 4x hintereinander \\
	\texttt{+ , * \{1,3\}, \{2\}} & von links nach rechts: mind. 1x, beliebig oft, 1 bis 3 Mal, genau 2 Mal \\ \hline
	\rowcolor{lightpink} \texttt{\$z = \dq alf@beta.com \dq\$z -match \dq (\textbackslash w*)(@)(\textbackslash w*).(com) \dq} & Capturing Groups: Jeder Ausdruck in Klammern kann über \texttt{\$matches[i]} abgerufen werden. $i$ steht für die Klammer (von 0 beginnend) \\
	\end{tabular} 
